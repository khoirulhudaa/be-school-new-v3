...
router.post('/', protect, upload.single('imageUrl'), async (req, res) => {
  try {
    ...
    // 🔥 VALIDASI LIMIT PLAN
    // =========================
    const plan = await BannerPricing.findOne({
      where: { planId: 'basic' } // sementara (nanti bisa dynamic)
    });

    const activeCount = await SponsorBanners.count({
      where: { schoolId, isActive: 1 }
    });

    if (plan && activeCount >= plan.maxBanners) {
      return res.status(400).json({
        success: false,
        message: `Maksimal ${plan.maxBanners} banner aktif untuk paket ${plan.name}`
      });
    }
